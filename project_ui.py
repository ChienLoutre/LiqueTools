...
def UI():
	...
	mc.frameLayout( label='Open Shot', cll = True, cl=True, cc= layout_refresh)

	# create layout number option menu
	mc.separator(h = 10, vis = False)
	shlo_numOptionMenu = mc.optionMenu('shlo_numOptionMenu', label = 'Sequence Number :    ', cc=shots_populateNumber, w = w)

	# populate  the layout option menu
	
	shot_layout_populateNumber()


	shlo_numOptionMenu = mc.optionMenu('sh_numOptionMenu', label = 'Shot Number :    ', cc=shots_populateVersions, w = w)

	# populate  the shots option menu
	
	shots_populateNumber()

	# create layout versions option menu
	mc.separator(h = 10, vis = False)
	sh_versionOptionMenu = mc.optionMenu('sh_versionOptionMenu', label = 'Version : ', w = w)

	# populate versions
	shots_populateVersions()

	# create the open_layout button
	mc.separator(h = 10, vis = False)
	mc.button(label='Open Shot', h = 30, c = open_shot, w = w, bgc = [1,0.73,0])
...
def shot_layout_populateNumber(*args):

	menuItems = mc.optionMenu('shlo_numOptionMenu', q = True, itemListLong = True)
	selectedNumber = mc.optionMenu('shlo_numOptionMenu', q = True, v = True)
	...
	for project in projects:
		mc.menuItem(label = project, parent = 'shlo_numOptionMenu')


def shots_populateNumber(*args):

	menuItems = mc.optionMenu('sh_numOptionMenu', q = True, itemListLong = True)
	selectedNumber = mc.optionMenu('sh_numOptionMenu', q = True, v = True)
	selectedLayout = mc.optionMenu('shlo_numOptionMenu', q = True, v = True)
	...
	projectPath = project_dir + '/prod/seq/' + selectedLayout + '/shots/'
	projects = os.listdir(projectPath)
	for project in projects:
		mc.menuItem(label = project, parent = 'sh_numOptionMenu')
...
def shots_populateVersions(*args):
	

	menuItems = mc.optionMenu('sh_versionOptionMenu', q = True, itemListLong = True)
	...
	selectedLayout = mc.optionMenu('shlo_numOptionMenu', q = True, v = True)
	selectedShot = mc.optionMenu('sh_numOptionMenu', q = True, v = True)
	
	if selectedShot != None:

		projectPath = project_dir + '/prod/seq/' + selectedLayout + '/shots/' + selectedShot + '/anim'
		files = os.listdir(projectPath)
		print(projectPath)
		if 'master' in files:
			files.remove('master')
	...
	for file in files:
		mc.menuItem(label = file, parent = 'sh_versionOptionMenu')
...
def open_shot(*args):
	...
	selectedLayout = mc.optionMenu('shlo_numOptionMenu', q = True, v = True)
	selectedShot = mc.optionMenu('sh_numOptionMenu', q = True, v = True)
	selectedVersion = mc.optionMenu('sh_versionOptionMenu', q = True, v = True)

	projectPath = project_dir + '/prod/seq/' + selectedLayout + '/shots/' + selectedShot + '/anim/' + selectedVersion
	selectedShotVersion = projectPath + '/anim.ma'

	go_on = mc.confirmDialog(title='Opening shot : ' + selectedShot + ' version ' + selectedVersion , message = "Your current file won't be saved, are you sure ?", button =['Yes','No'], defaultButton='Yes', cancelButton='No', dismissString='No')

	if go_on != 'Yes':

		
		sys.exit('Opening of shot ' + selectedShot + ' aborted.')

	print(selectedShotVersion + ' is opening...')

	mc.file(selectedShotVersion, open=True, f = True)

	print(selectedShotVersion + ' opened successfully !')
...
def g_asset_export(*args):
	...
	folder_to_export = mc.ls(sl=True)

	if len(folder_to_export) > 5:
		raise ValueError('Too many arguments.')

	for each in folder_to_export:

		if each not in check_name:

			folder_to_export.remove(each)
	...
	if 'seq' in charvalue:
		...
		if result == 'OK':
			...
			etg.g_abc_seq_export(mode='startend',qst=fst,qend=end, check_name=folder_to_export)

			mc.warning('ABC exported and CLEAN.')
		else:
			etg.g_abc_seq_export(mode='currentframe',qst=0,qend=0,check_name=folder_to_export)
			mc.warning('ABC exported and CLEAN.')
	else:
...
def save_master(*args):
	...
	save.master(comments=comments, merge_refs=False, bin_or_ascii='mayaBinary', ask_version=True)
