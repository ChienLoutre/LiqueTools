...
def create_limb(limb_pos, limb_name, aim_axis, up_axis):
    ...
    joint_zero = zero_out(jnt_sel[0])
    
    # faire tourner le start_jnt vers le mid_jnt
    aimCstr = mc.aimConstraint(jnt_sel[1],jnt_sel[0], aim= aim_axis, u = up_axis)
    temp_x = mc.getAttr(jnt_sel[0] + '.rotateX')
    temp_y = mc.getAttr(jnt_sel[0] + '.rotateY')
    temp_z = mc.getAttr(jnt_sel[0] + '.rotateZ')
    
    mc.delete(aimCstr)
        
    mc.setAttr(jnt_sel[0] + '.jointOrientX', temp_x)
    mc.setAttr(jnt_sel[0] + '.jointOrientY', temp_y)
    mc.setAttr(jnt_sel[0] + '.jointOrientZ', temp_z)
    
    mc.xform(jnt_sel[0], ro=(0,0,0))
    
    
    # faire tourner le mid_jnt vers le end_jnt
    aimCstr = mc.aimConstraint(jnt_sel[2],jnt_sel[1], aim= aim_axis, u = up_axis)
    
    temp_x = mc.getAttr(jnt_sel[1] + '.rotateX')
    
    print('temp_x == ' + str(temp_x))
    if int(temp_x) == -90:
        
        temp_x = -(temp_x)
        print('valeur de temp_x == ' + str(temp_x))
            
    temp_y = mc.getAttr(jnt_sel[1] + '.rotateY')
    temp_z = mc.getAttr(jnt_sel[1] + '.rotateZ')
    
    mc.delete(aimCstr)
    
    mc.parent(jnt_sel[2], jnt_sel[1])
    #mc.xform(jnt_sel[2], ro=(0,0,0))
    #mc.parent(jnt_sel[2], w=True)
    
    
    mc.setAttr(jnt_sel[1] + '.jointOrientX', temp_x)
    mc.setAttr(jnt_sel[1] + '.jointOrientY', temp_y)
    mc.setAttr(jnt_sel[1] + '.jointOrientZ', temp_z)
